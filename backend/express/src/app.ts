...
class App {
    ...
    private async config(): Promise<void> {
        ...
        if (process.env.INFLUX_LOGGING === 'true') {
            this.app.use(
                (request: Request, response: Response, next: NextFunction) => loggingMiddleware.logRequests(request, response, next)
            );
        }
        ...
        if (process.env.INFLUX_LOGGING === 'true') {
            // Log all errors to Influx
            this.app.use(
                (error: unknown, request: Request, response: Response, next: NextFunction) =>
                    loggingMiddleware.logErrors(error, request, response, next)
            );
        }
